...
def get_function(func_string: str) -> callable:
    ...
    :usage:
        ..> get_function(
        ...     'application.vendor.replenishment.run_prod_cls_criteria'
        ... )
    """
...
class Profile(BaseUpdatableModel):
    ...
    def prepare_for_each_foreign_key(cls, value):
        ...
        for v in ['name', 'ref_table']:
            if v not in value:
                raise ValueError(
                    f'foreign key value does not contain {v}'
                )
...
class Table(BaseUpdatableModel):
    ...
    def validate_name_flag(
            self,
            flag: Optional[Union[str, bool]]
    ) -> bool:
