...
# module function ------------------------------------------------------------------------
def run_min_max_service_level(input_df: pd.DataFrame, service_level_type: str, service_level_round: int):
    """
    ...
    for value in input_values:
        ...
        result_list: str = ', '.join([
            str(round(stats.norm.ppf(float(_) / service_level_divide), service_level_round)) for _ in value[:-1]
        ])
...
def run_prod_cls_criteria(input_df: pd.DataFrame):
    ...
    for value in input_values:
        ...
        if not math.isclose(sum(input_criteria), 100) and not math.isclose(sum(input_criteria), 1):
            input_criteria_filter: list = [_ / 100 for _ in input_criteria] if sum(input_criteria) > 100 \
                else input_criteria
            class_a, class_b, class_c = input_criteria_filter
            if math.isclose((class_c - class_b) + (class_b - class_a) + class_a, 1) and math.isclose(class_c, 1):
                result_list: str = ', '.join([str(class_a), str(class_b), str(class_c)])
                result_str: str = f"({result_list}, {result_version}, 'Y', '{update_datetime}')"
                values.append(result_str)
            else:
                values.append('')
        else:
            input_criteria_filter: list = [_ / 100 for _ in input_criteria] if math.isclose(sum(input_criteria), 100) \
                else input_criteria
            class_a, class_b, class_c = input_criteria_filter
    ...
    logger.info("Validate product class criteria value success, it will update to database")
    return ", ".join([_ for _ in values if _ != '']) if any(_ != '' for _ in values) else ""
