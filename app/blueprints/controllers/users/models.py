...
class User(db.Model):
    ...
    image_file = db.Column(
        "image_file", db.String(256), nullable=False, default="default.jpg"
    )
    active = db.Column("active_flag", db.Boolean, nullable=False, default=True)
    register_date = db.Column(
        "register_date", db.DateTime(timezone=True), default=datetime.now
    )
    ...
    def is_admin(self) -> bool:
        return self.is_authenticated and any(
            "Administrator" in role.name for role in self.roles
        )

    def get_reset_token(self, expires_hour: int = 1):
        expires_hour: int = current_app.config.get(
            "RESET_TOKEN_EXPIRE_HOURS", expires_hour
        )
        return jwt.encode(
            {
    ...
    def has_permission(self, permission):
        for role in self.roles:
            ...
            if policies and any(
                permission in policy.allowed_routes for policy in policies
            ):
                return True
        return False
...
role_policy = Table(
    ...
    db.Column(
        "role_id",
        db.Integer,
        db.ForeignKey("ctr_web_role.role_id"),
        nullable=False,
    ),
    db.Column(
        "policy_id",
        db.Integer,
        db.ForeignKey("ctr_web_policy.policy_id"),
        nullable=False,
    ),
    db.Column(
        "update_date", db.DateTime(timezone=True), default=datetime.now()
    ),
    # autoload=True,
    extend_existing=True,
    autoload_with=db.engine,
...
class Role(db.Model):
    ...
    role_name = db.Column(
        "role_name", db.String(128), nullable=False, unique=True
    )
...
user_group = db.Table(
    ...
    db.Column(
        "group_id", db.BigInteger, db.ForeignKey("ctr_web_group.group_id")
    ),
    db.Column(
        "update_date",
        db.DateTime(timezone=True),
...
group_to_group = db.Table(
    ...
    db.Column("map_id", db.BigInteger, primary_key=True),
    db.Column(
        "parent_id", db.BigInteger, db.ForeignKey("ctr_web_group.group_id")
    ),
    db.Column(
        "child_id", db.BigInteger, db.ForeignKey("ctr_web_group.group_id")
    ),
    db.Column(
        "update_date",
        db.DateTime(timezone=True),
...
class Group(db.Model):
    ...
    group_name = db.Column(
        "group_name", db.String(128), unique=True, nullable=False
    )
    description = db.Column("description", db.Text, nullable=True)
    register_date = db.Column(
        "register_date", db.DateTime(timezone=True), default=datetime.now()
    ...
    users = db.relationship(
        ...
        lazy="select",
        uselist=True,
        backref=db.backref("groups", lazy="select", uselist=True),
    )
