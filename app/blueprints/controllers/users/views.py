...
def register_get():
    ...
    return render_template(
        "users/users_base.html", target="register", form=form
    )
...
def login_post():
    ...
    if form.validate_on_submit():
        ...
        if user and bcrypt.check_password_hash(
            ...
            _response = make_response()
            _response.headers["HX-Redirect"] = request.args.get(
                "next"
            ) or url_for("home")
            _response.status_code = 200
            return _response
...
def account():
    ...
    return render_template(
        "users/account.html", image_file=image_file, form=form
    )
...
def reset_request():
    ...
    return render_template(
        "users/users_base.html", target="reset_request", form=form
    )
