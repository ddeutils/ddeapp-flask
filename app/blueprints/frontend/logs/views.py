...
def log_data_chart():
    ...
    logs_df = logs_df.reset_index(drop=True).astype(
        {"process_number_put": int, "process_time": int}
    )
    logs_df[["process_type_bg", "process_type"]] = logs_df[
        "process_type"
    ].str.split("|", n=1, expand=True)
    logs_df[["process_module", "process_module_value"]] = logs_df[
        "process_module"
    ].str.split("|", n=1, expand=True)
    logs_df["update_date"] = pd.to_datetime(logs_df["update_date"]).dt.date
    logs_df["update_date"] = pd.to_datetime(
        logs_df["update_date"]
    ) + pd.offsets.MonthBegin(1)

    logs_df = logs_df.groupby(
        ["process_type_bg", "update_date"], sort=False, as_index=False
    ).agg(
        {
    ...
    data = list(
        filter(lambda row: row["process_type_bg"] == "foreground", data)
    )
    return jsonify(
        {
...
def log_data():
    ...
    for _ in range(6):
        ...
        if sn := request.args.get(f"multiSort[{_}][sortName]", "", type=str):
            multi_sort.append(
                {
                    "sortName": sn,
                    "sortOrder": request.args.get(
                        f"multiSort[{_}][sortOrder]", "asc", type=str
                    ),
                }
            )
        else:
    ...
    if (limit := request.args.get("limit", 0, type=int)) > 0:
        ...
        _logs = searched_task_log.order_by(*sorting).paginate(
