...
class FormSetup(FormValidate):
    ...
    @classmethod
    def co_validate_table_name_and_pipeline_name(
        cls, table_name, pipeline_name
    ):
...
class FormRetention(FormValidate):
    ...
    def validate_backup_table(cls, backup_table):
        if not validate_table(backup_table, optional=False):
            raise ValidateFormsError(
                "backup_table",
                backup_table,
                f"{backup_table!r} does not exists in catalog",
            )
    ...
    def validate_backup_schema(cls, backup_schema):
        if not validate_parameter(
            backup_schema,
            [os.getenv("AI_SCHEMA", "ai"), os.getenv("MAIN_SCHEMA", "public")],
        ):
            raise ValidateFormsError(
                "backup_schema",
                backup_schema,
                f"{backup_schema!r} does not exists in database",
            )
