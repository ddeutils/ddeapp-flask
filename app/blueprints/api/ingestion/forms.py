...
class FormIngest(ContentValidate):
    ...
    def validate_update_date(cls, update_date):
        if validate_update_date(update_date):
            raise ValidateFormsError(
                "update_date",
                update_date,
                "it does not ISO date format: YYYY-mm-dd HH:MM:SS'",
            )
    ...
    def validate_payloads(cls, payloads):
        if (
            not (payloads and isinstance(payloads, (dict, list)))
            and request.method == "PUT"
        ):
            raise ValidateFormsError(
                "data",
                payloads,
                "`data` does not exists in data-raw or type of `data` does not support",
            )
