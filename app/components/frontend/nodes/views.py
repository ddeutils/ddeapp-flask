...
def all_node():
    ...
    _nodes = Node.query.order_by(Node.update_date.desc()).paginate(page=page, per_page=25)
...
def all_node_data():
    ...
    if search := request.args.get('search[value]'):
        query = query.filter(db.or_(
            Node.name.like(f'%{search}%'),
            Node.type.like(f'%{search}%')
        ))
...
def node(node_name):
    ...
    logs = NodeLog.query \
        .filter_by(name=_node.name) \
        .order_by(NodeLog.update_date.desc()) \
        .paginate(page=page, per_page=25)

    return render_template(
        'nodes/node.html', node=_node, logs=logs, graphJSON=node_log_data(_node.name)
    )
...
def node_log_data(name: str, selected: Optional[str] = None):
    ...
    logs_df = pd.read_sql_query(
        NodeLog.query.filter_by(name=name).order_by(NodeLog.update_date.desc()).statement,
    ...
    if selected == 'Week':
        ...
        logs_df['run_date'] = pd.to_datetime(logs_df['run_date']) + pd.offsets.MonthBegin(1)
    elif selected == 'Year':
        logs_df['run_date'] = pd.to_datetime(logs_df['run_date']) + pd.offsets.YearBegin(1)
...
def pipeline():
    ...
    if 'Hx-Request' in request.headers:
        ...
        return render_template(
            'nodes/partials/pipelines.html', pipelines=pipelines, paginate_flag=paginate_flag
        )
